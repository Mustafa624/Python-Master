...
async function initializePyodide() {
    ...
    try {
        ...
        while (attempts < maxAttempts) {
            ...
            // Method 1: Direct from window
            if (typeof window.loadPyodide !== 'undefined') {
                const func = window.loadPyodide;
                // Check if it's a function and has the expected signature (not our function)
                if (typeof func === 'function' && func !== initializePyodide) {
                    // Check if it's Pyodide's function by checking if it accepts indexURL
                    try {
            ...
            if (pyodideScript.readyState === 'complete' || pyodideScript.readyState === 'loaded') {
                await new Promise(resolve => setTimeout(resolve, 500));
                if (typeof window.loadPyodide !== 'undefined') {
                    const func = window.loadPyodide;
                    if (typeof func === 'function' && func !== initializePyodide) {
